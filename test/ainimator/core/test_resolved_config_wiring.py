...
from ainimator.core.resolved_config import (
...
def test_write_resolved_config_overwrites_existing(
    ...
    import yaml
    from ainimator.core.config_schema import V2TrainingConfigSchema
...
def test_run_overfit_writes_resolved_config(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "ainimator.training.training_v2.loadDatasetSample",
        ) as mockLoad,
        patch(
            "ainimator.training.training_v2.buildTrainingComponents",
        ) as mockBuild,
        patch(
            "ainimator.training.training_v2.trainStep",
        ) as mockStep,
        patch(
            "ainimator.training.training_v2.saveCheckpointV2",
        ),
    ):
...
def test_run_full_training_writes_resolved_config(
    ...
    with (
        patch(
            "ainimator.training.full_training_v2"
        ...
        patch(
            "ainimator.training.full_training_v2.iterBatches",
            return_value=iter([]),
        ),
        patch(
            "ainimator.training.full_training_v2.validateEpoch",
            return_value={
                "loss_total": 0.5,
                "loss_bone": 0.25,
                "loss_global": 0.25,
